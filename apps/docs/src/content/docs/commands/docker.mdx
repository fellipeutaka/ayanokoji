...
- **Independent environment synchronization** - Runs only after a successful Compose write; declining it keeps the operation successful, while a failure reports both outcomes without rolling back the Compose file
...
- **Independent environment cleanup** - Declining cleanup keeps the operation successful; a cleanup failure reports both outcomes without rolling back the Compose file
...
- Symlinked or changed Compose paths are rejected without overwriting the file; a stale-document conflict is not reread or retried, so rerun the command explicitly.
...
- Discovery, parsing, validation, mutation, serialization, and Compose write failures are reported as nonzero outcomes without partially writing the Compose document; environment failures explicitly report when the Compose write already succeeded.
