...
- [`erasableSyntaxOnly: true`](https://www.typescriptlang.org/tsconfig/#erasableSyntaxOnly) - Limits TypeScript syntax to features that can be easily erased to JavaScript. Disallows `enums`, `namespaces` with runtime code, `parameter properties` in classes, and non-ECMAScript `import/export` assignments.
  ```ts
...
- [`composite: true`](https://www.typescriptlang.org/tsconfig/#composite) - Tells TypeScript to emit `.tsbuildinfo` files. This tells TypeScript that your project is part of a monorepo, and also helps it to cache builds to run faster.
- [`declaration: true`](https://www.typescriptlang.org/tsconfig/#declaration) - Tells TypeScript to emit `.d.ts` files. This is needed so that libraries can get autocomplete on the `.js` files you're creating.
- [`declarationMap: true`](https://www.typescriptlang.org/tsconfig/#declarationMap) - Generates source maps for `.d.ts` files. These are needed so that when consumers of your libraries are debugging, they can jump to the original source code using go-to-definition.
